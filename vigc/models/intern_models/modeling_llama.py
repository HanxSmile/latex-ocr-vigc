...
try:
    from .converted_llama_modules import ConvertedLlamaAttention
    from .intern_converted_llama_modules import InternConvertedLlamaAttention
except:
    print('import ConvertedLlamaAttention fail!')
...
class LlamaDecoderLayer(nn.Module):
    def __init__(self, config: LlamaConfig):
        super().__init__()
        self.hidden_size = config.hidden_size
        if hasattr(config, 'converted_llm') and config.converted_llm:
            self.self_attn = ConvertedLlamaAttention(config=config)
        elif hasattr(config, 'intern_converted_llm') and config.intern_converted_llm:
...
class LlamaForCausalLM(LlamaPreTrainedModel):
    ...
    @classmethod
    def from_pretrained(cls, pretrained_model_name_or_path, llm_cfg=None, *model_args, **kwargs):
        if llm_cfg:
    ...
    def forward(
        ...
        if labels is not None:
            ...
            if self.sp_id >= 0:
                ...
                ori_mask = (shift_labels < self.config.vocab_size - self.ex_size).float()
                ori_mask = ori_mask * (shift_labels >= 0).float()
                local_mask = (shift_labels >= self.config.vocab_size - self.ex_size).float()
